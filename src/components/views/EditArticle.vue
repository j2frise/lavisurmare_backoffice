          ...
          <div class="box box-info">
            ...
            <div class="box-body">
                ...
                <div class="col-xs-12 col-md-10">
                  <h4>
                       <router-link tag="a" :to="'/in/article/'+article.sub_id">
                            <a>
                            <i class="fa fa-arrow-left"></i>
                            retour
                            </a>
                        </router-link> | 
                      Modifier un article dans le chapitre "<span style="text-transform: uppercase" class="text-primary">{{ article.sub_sTitre }}</span>"</h4>
                  <form @submit.prevent="editArticle" method="POST" enctype="multipart/form-data">
                    <div class="form-group col-xs-12 col-md-6">
                      <label class="control-label">Thème</label>
                      <select class="form-control require" name="theme" @change="choiceTheme">
                          <option value="">-------- Choisissez ----------</option>
                          <option v-for="(element, index) in theme" :key="index" :value="element.tya_id" :selected="article.tya_id == element.tya_id">{{ element.tya_sLib }}</option>
                      </select>
                    </div>
                    <div class="form-group col-xs-12 col-md-6">
                      <label class="control-label">Titre</label>
                      <input class="form-control require" name="title" :value="article.art_sTitre" type="text">
                      <input name="id" :value="article.art_id" type="hidden">
                      <input name="img1_m" :value="article.art_sImage_1" type="hidden">
                      <input name="img2_m" :value="article.art_sImage_2" type="hidden">
                      <input name="v1_m" :value="article.art_sVideo_1" type="hidden">
                      <input name="v2_m" :value="article.art_sVideo_1" type="hidden">
                      <input name="audio_m" :value="article.art_sAudio" type="hidden">
                      <input name="idSub" :value="article.sub_id" type="hidden">
                    </div>
                    <div class="form-group col-xs-12 col-md-6" v-if="img1">
                      <label class="control-label">Image principale</label>
                      <input class="form-control img1" accept="image/*" name="img1" placeholder="" type="file">
                    </div>
                    <div class="form-group col-xs-12 col-md-6" v-if="img2">
                      <label class="control-label">Image sécondaire</label>
                      <input class="form-control img2" accept="image/*" name="img2" placeholder="" type="file">
                    </div>
                    <div class="form-group col-xs-12 col-md-6" v-if="v1">
                      <label class="control-label">Vidéo principale</label>
                      <input class="form-control v1" accept="video/mp4, video/ogg, video/webm" name="v1" placeholder="" type="file">
                    </div>
                    ...
                    <div class="form-group col-xs-12 col-md-6" v-if="p1">
                      ...
                      <textarea rows="4" class="form-control require" :value="article.art_sPar_1" name="p1"></textarea>
                    </div>
                    <div class="form-group col-xs-12 col-md-6" v-if="p2">
                      <label class="control-label">2ème Paragraphe</label>
                      <textarea rows="4" class="form-control require" :value="article.art_sPar_2" name="p2"></textarea>
                    </div>
                    <div class="form-group col-xs-12 col-md-6" v-if="p3">
                      <label class="control-label">3ème Paragraphe</label>
                      <textarea rows="4" class="form-control require" :value="article.art_sPar_3" name="p3"></textarea>
                    </div>
                    <div class="form-group col-xs-12 col-md-6" v-if="p4">
                      <label class="control-label">4ème Paragraphe</label>
                      <textarea rows="4" class="form-control require" :value="article.art_sPar_4" name="p4"></textarea>
                    </div>
...
export default {
  ...
  methods: {
    getArticle() {
      api
        .request('get', '/get/index.php?article&id=' + this.id)
        .then(response => {
          var data = response.data
          if (!data.error) {
            this.article = data.return
            this.choiceThemeDefault(this.article.tya_id)
          } else {
    ...
    editArticle() {
      ...
      if (this.count !== 0) {
        ...
        api
          .request('post_file', '/editArticle/index.php', formData)
          .then(response => {
            var data = response.data
            this.success = false
            if (!data.error) {
              if (data.return === 'exist') {
                ...
                var selectFile = target.querySelectorAll('input[name=file]')
                if (selectFile) {
